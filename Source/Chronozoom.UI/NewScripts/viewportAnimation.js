    ...
    (function (ViewportAnimation) {
        ...
        */
        function PanZoomAnimation(startViewport) {
            ...
            startViewport.aspectRatio, startViewport.width, startViewport.height, new CZ.Viewport.VisibleRegion2d(startVisible.centerX, startVisible.centerY, startVisible.scale));
            ...
            this.setTargetViewport = function (estimatedEndViewport) {
                ...
                if(this.pathLeng < 1e-1) {
                    ...
                    if(estimatedVisible.scale == prevVis.scale) {
                        //if the scales are the same as well the animation can be stoped
                        this.isActive = false;
                    }
                } else {
        ...
        */
        function EllipticalZoom(startVisible, endVisible) {
            ...
            if(Math.abs(u0 - u1) > this.imprecision) {
                ...
                if(this.r0 == -Infinity) {
                    //happens when the double precision of r0 calculation yields infinity
                    this.r0 = -Math.log(2 * b0);
                }//instead approximating with the first element of the teylor series
                
                this.r1 = Math.log(-b1 + Math.sqrt(b1 * b1 + 1));
                if(this.r1 == -Infinity) {
                    // the same reaction on Infinity as in r0 calculation
