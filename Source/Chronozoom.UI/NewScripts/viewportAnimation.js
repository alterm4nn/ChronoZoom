...
function PanZoomAnimation(startViewport) {
    ...
    this.isActive = true;
    this.type = "PanZoom";
    this.startViewport = new Viewport2d(startViewport.aspectRatio, startViewport.width, startViewport.height, new VisibleRegion2d(startVisible.centerX, startVisible.centerY, startVisible.scale));
    ...
    this.setTargetViewport = function (estimatedEndViewport) {
        ...
        this.startViewport = new Viewport2d(this.previousFrameViewport.aspectRatio, this.previousFrameViewport.width, this.previousFrameViewport.height, new VisibleRegion2d(prevVis.centerX, prevVis.centerY, prevVis.scale));
...
function EllipticalZoom(startVisible, endVisible) {
    ...
    var ro = 0.1 * ellipticalZoomZoomoutFactor;
    this.ro = ro;
    var u0 = 0;
    this.u0 = u0;
    var u1 = this.pathLen;
    ...
    if(Math.abs(u0 - u1) > this.imprecision) {
        ...
        this.duration = ellipticalZoomDuration / 300 * this.S;
    } else {
