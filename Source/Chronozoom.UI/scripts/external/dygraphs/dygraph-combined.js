...
Dygraph.prototype.yAxisRange = function (a) {
    ...
    var b = this.axes_[a];
    return [
        b.computedValueRange[0], 
        b.computedValueRange[1]
    ];
};
        ...
        if(this.fractions_) {
            for(k = 1; k < d.length; k++) {
                ...
                if(g.length != 2) {
                    ...
                    h[k] = [
                        this.parseFloat_(g[0], l, e), 
                        this.parseFloat_(g[1], l, e)
                    ];
                }
            }
        } else {
            if(this.attr_("errorBars")) {
                ...
                for(k = 1; k < d.length; k += 2) {
                    h[(k + 1) / 2] = [
                        this.parseFloat_(d[k], l, e), 
                        this.parseFloat_(d[k + 1], l, e)
                    ];
                }
            } else {
                if(this.attr_("customBars")) {
...
Dygraph.Interaction.movePan = function (b, k, c) {
    ...
    if(c.is2DPan) {
        ...
        for(var j = 0; j < k.axes_.length; j++) {
            ...
            if(m) {
                e.valueWindow = [
                    Math.pow(Dygraph.LOG_SCALE, n), 
                    Math.pow(Dygraph.LOG_SCALE, l)
                ];
            } else {
                e.valueWindow = [
                    n, 
...
Dygraph.Interaction.moveTouch = function (n, q, d) {
    ...
    if(d.touchDirections.x) {
        q.dateWindow_ = [
            j.dataX - m.dataX + (d.initialRange.x[0] - j.dataX) / w, 
            j.dataX - m.dataX + (d.initialRange.x[1] - j.dataX) / w
        ];
        u = true;
    }
    if(d.touchDirections.y) {
        for(p = 0; p < 1; p++) {
            ...
            if(s) {
                ...
                b.valueWindow = [
                    j.dataY - m.dataY + (d.initialRange.y[0] - j.dataY) / c, 
                    j.dataY - m.dataY + (d.initialRange.y[1] - j.dataY) / c
                ];
                u = true;
            }
        }
