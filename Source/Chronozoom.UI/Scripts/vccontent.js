...
function CanvasTimeline(vc, layerid, id, vx, vy, vw, vh, settings, timelineinfo) {
    ...
    */
    this.render = function (ctx, visibleBox, viewport2d, size_p, opacity) {
        ...
        //currently observed tracing
        var isVisibleInTheRectangle = !(p.x > 0 || p.y > 0 || (p2.x < viewport2d.width && (this.x + this.width < 0)) || p2.y < viewport2d.height);
    ...
    this.render = function (ctx, visibleBox, viewport2d, size_p, opacity) {
        ...
        if (!this.settings.wrapText) {
            ...
            for (var i = 0; i < this.text.length; i++) {
                ctx.fillText(this.text[i], p.x, bp);
                bp += fontSize * k;
            }
        }
    };
    ...
    this.changeZoomLevel = function (curZl, newZl) {
        if (newZl >= contentItemShowContentZoomLevel) { // building content for an infodot
            ...
            if (contentItem.mediaType === 'image') {
                imageElem = addImage(container, layerid, mediaID, vx + leftOffset, mediaTop, contentWidth, mediaHeight, contentItem.mediaUrl);
            }
...
function CanvasInfodot(vc, layerid, id, time, vyc, radv, contentItems, infodotDescription) {
    ...
    root.changeZoomLevel = function (curZl, newZl) {
        if (newZl >= infodotShowContentZoomLevel) { // building content for an infodot
            ...
            if (infodot.contentItems.length > 0) {
                ...
                if (items) {
                    ...
                    bibl.onmouseclick = function (e) {
                        this.vc.element.css('cursor', 'default');
                        showBibliography({ infodot: infodotDescription, contentItems: infodot.contentItems });
                        return true;
                    };
            ...
            if (infodot.contentItems.length == 0) return null;
            var zl = newZl;
