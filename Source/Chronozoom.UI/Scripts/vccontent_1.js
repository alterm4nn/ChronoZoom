...
var render = function (element, contexts, visibleBox_v, viewport2d, opacity) {
    ...
    if (sz.y <= renderThreshold || (element.width != 0 && sz.x <= renderThreshold)) { // (width != 0): to render text first time, since it measures its width on first render only
        if (element.isRendered) turnIsRenderedOff(element);
        return;
    }
...
function CanvasTimeline(vc, layerid, id, vx, vy, vw, vh, settings, timelineinfo) {
	...
	*/
    this.render = function (ctx, visibleBox, viewport2d, size_p, opacity) {
        ...
        var isCenterInside = viewport2d.visible.centerX - timelineCenterOffsetAcceptableImplicity <= this.x + this.width &&
                             viewport2d.visible.centerX + timelineCenterOffsetAcceptableImplicity >= this.x &&
                             viewport2d.visible.centerY - timelineCenterOffsetAcceptableImplicity <= this.y + this.height &&
                             viewport2d.visible.centerY + timelineCenterOffsetAcceptableImplicity >= this.y;

        // is timeline inside "breadcrumb offset box"
        var isVisibleInTheRectangle = ((p.x < timelineBreadCrumbBorderOffset && p2.x > viewport2d.width - timelineBreadCrumbBorderOffset) ||
                        (p.y < timelineBreadCrumbBorderOffset && p2.y > viewport2d.height - timelineBreadCrumbBorderOffset));
	...
	*/
    this.render = function (ctx, visibleBox, viewport2d, size_p, opacity) {
        ...
        if (!this.settings.wrapText) {
            ...
            drawText(this.text, ctx, p.x, p.y, fontSize, this.settings.fontName);
        } else { // multiline text
        ...
        for (; --n >= 0;) {
            if (this.imageSources[n].zoomLevel <= newZoomLevel) {
                ...
                return {
                    ...
                    content: new CanvasImage(vc, layerid, id + "@" + this.imageSources[n].zoomLevel, this.imageSources[n].imageSource, vx, this.y, vw, vh, onload)
                };
            }
        }
        return null;
...
function ContentItem(vc, layerid, id, vx, vy, vw, vh, contentItem) {
    ...
    this.mediaTop = this.y + verticalMargin;  //vy + titleHeight + 2 * verticalMargin;
    var sourceVertMargin = verticalMargin * 0.4;
    this.sourceTop = this.mediaTop + mediaHeight + sourceVertMargin;
    var sourceRight = this.x + vw - leftOffset;
    var sourceHeight = vh * contentItemSourceHeight * 0.8;
    this.titleTop = this.sourceTop + verticalMargin + sourceHeight;

    // Bounding rectangle
    var rect = addRectangle(this, layerid, id + "__rect__", this.x, this.y, vw, vh,
                            {
    ...
    this.changeZoomLevel = function (curZl, newZl) {
        if (newZl >= contentItemShowContentZoomLevel) { // building content for an infodot
            ...
            var container = new ContainerElement(vc, layerid, id + "__content", this.x, this.y, vw, vh);

            // Media
            var mediaID = id + "__media__";
            var imageElem = null;
            if (contentItem.mediaType === 'image') {
                imageElem = addImage(container, layerid, mediaID, this.x + leftOffset, this.mediaTop, contentWidth, mediaHeight, contentItem.mediaUrl);
                ...
                addVideo(container, layerid, mediaID, contentItem.mediaUrl, this.x + leftOffset, this.mediaTop, contentWidth, mediaHeight, mediaContentElementZIndex);
            }
            else if (contentItem.mediaType === 'audio') {
                this.mediaTop += contentItemAudioTopMargin * vh;
                mediaHeight = vh * contentItemAudioHeight;
                addAudio(container, layerid, mediaID, contentItem.mediaUrl, this.x + leftOffset, this.mediaTop, contentWidth, mediaHeight, mediaContentElementZIndex);
            }
            else if (contentItem.mediaType === 'pdf') {
                addPdf(container, layerid, mediaID, contentItem.mediaUrl, this.x + leftOffset, this.mediaTop, contentWidth, mediaHeight, mediaContentElementZIndex);
            }

            // Title
            var titleText = contentItem.title;
            addText(container, layerid, id + "__title__", this.x + leftOffset, this.titleTop, this.titleTop + titleHeight / 2.0,
            ...
            if (sourceText) {
                ...
                addSourceText(this.x + leftOffset, contentWidth, this.sourceTop);
            }


            // Description
            var descrTop = this.titleTop + titleHeight + verticalMargin;
            addScrollText(container, layerid, id + "__description__", this.x + leftOffset, descrTop,
                            contentWidth,
                            descrHeight,
...
function CanvasInfodot(vc, layerid, id, time, vyc, radv, contentItems, infodotDescription) {
    ...
    this.onmouseenter = function (e) {
        ...
        $(".bubbleInfo span").text(this.infodotDescription.title);
        this.panelWidth = $('.bubbleInfo').outerWidth(); // complete width of tooltip panel
        this.panelHeight = $('.bubbleInfo').outerHeight(); // complete height of tooltip panel        
    ...
    var infodot = this;
    var root = new CanvasDynamicLOD(vc, layerid, id + "_dlod", infodot.time - innerRad, this.vyc - innerRad, 2 * innerRad, 2 * innerRad);
    ...
    root.changeZoomLevel = function (curZl, newZl) {
        ...
        if (newZl >= infodotShowContentThumbZoomLevel && newZl < infodotShowContentZoomLevel) {
            ...
            if (infodot.contentItems.length > 0) {
                ...
                var items = buildVcContentItems(infodot.contentItems, infodot.time, infodot.vyc, innerRad, vc, layerid);
                if (items)
            ...
            var titleTop = infodot.vyc - centralSquareSize - titleHeight;
            var title = '';

            if (infodot.infodotDescription && infodot.infodotDescription.title && infodot.infodotDescription.date) {
                title = infodot.infodotDescription.title + '\n(' + infodot.infodotDescription.date + ')';
            }
            ...
            //    this.vc.element.css('cursor', 'default');
            //    showBibliography({ infodot: infodot.infodotDescription, contentItems: infodot.contentItems }, contentItem, id + "__bibliography");
            //    return true;
            //};
            ...
            //    //bid[0] - source string
            //    //bid[1] - found match
            //    showBibliography({ infodot: infodot.infodotDescription, contentItems: infodot.contentItems }, contentItem, bid[1]);
            //}
            ...
            if (zl < contentItemThumbnailMinLevel) {
                return {
                    zoomLevel: zl,
                    content: new ContainerElement(vc, layerid, id + "__empty", infodot.time, infodot.vyc, 0, 0)
                };
            }
            ...
            return {
                zoomLevel: zl,
                content: new CanvasImage(vc, layerid, id + "@" + zl, thumbnailUri, infodot.time - l / 2.0, infodot.vyc - l / 2.0, l, l)
            };
        }
    ...
    var xlt0 = -_wc / 2 * radv + infodot.time;
    var ylt0 = -_hc / 2 * radv + infodot.vyc;
    var xlt1 = _wc / 2 * radv + infodot.time;
    var ylt1 = _hc / 2 * radv + infodot.vyc;
