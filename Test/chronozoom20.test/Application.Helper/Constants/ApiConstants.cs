namespace Application.Helper.Constants
{
    public class ApiConstants
    {
        public const string EndpointLocator = "{0}/api/{1}";
        public const string CosmosGuidTemplate = "bdc1ceff-76f8-4df4-ba72-96b353991314";

        public const string SuperCollectionUrl = "sandbox/sandbox/";
        public const string TimelineApiServiceUrl = SuperCollectionUrl + "timeline";
        public const string ExhibitApiServiceUrl = SuperCollectionUrl + "exhibit";
        public const string TourApiServiceUrl = SuperCollectionUrl + "tours";
    }
}